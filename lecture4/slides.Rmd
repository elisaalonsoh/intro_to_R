...
source(here::here("site_elements", "style.R"))
...
```{r, eval = F}
rio::import(here::here("lecture4", "slides_elements", "data", "ligue1.csv"))
                                                    #
                                                    #
                                                    #
                                                    #
...
ggplot(rio::import(here::here("lecture4", "slides_elements", "data", "wid.csv")))                                         # Data
                                                                    #
```
...
```{r, eval = F, fig.width = 8, fig.height = 4.5, out.width = "60%"}
ggplot(rio::import(here::here("lecture4", "slides_elements", "data", "wid.csv")), aes(x = inc_head, y = top1))            # Data & aesthetics
                                                                    #
```
...
<img src = "slides_elements/figures/starbucks_top.png" width = "399" style = "margin-top:-15.25cm !important;margin-left:18.6cm;margin-bottom:19cm"/>

---

### What we've seen so far

#### Write reports with `Quarto`
...
starbucks <- rio::import(here::here("lecture4", "slides_elements", "data", "starbucks.csv")) # Load data
```
...
<img src = "slides_elements/figures/starbucks_mid.png" width = "399" style = "margin-top:-15.25cm !important;margin-left:18.6cm;margin-bottom:15cm"/>
...
<div  style = "position:absolute!important;top:0px!important:left:0px;">
<p style = "margin-bottom:.5cm;"></p>
<img src = "slides_elements/figures/starbucks.png" width = "399" style = "margin-left:18.6cm;"/>
</div>
...
<ul style = "margin-left:1.5cm;list-style: none">
  <li><b>2. Getting to the right model</b></li>
  <ul style = "list-style: none">
...
```{r}
ggcurve <- rio::import(here::here("lecture4", "slides_elements", "data", "ggcurve.csv"))
str(ggcurve)
```
...
<ul>
  ...
  <ul>
    <li><b>ige:</b> The intergenerational income elasticity, the higher the closer child income to parent income</li>
    <li><b>gini:</b> The Gini coefficient of income inequality, the higher the more concentrated the income</li>
  </ul>
</ul>
...
<ul>
  <li>You are already familiar with univariate <b>regressions</b> \(y = \alpha + \beta x + \varepsilon\)</li>
</ul>
...
```{r}
asec <- rio::import(here::here("lecture4", "slides_elements", "data", "asec.csv"))
str(asec)
```
...
<ul>
  <li>Once again the <b>constant</b> is the <b>average</b> \(y\) for the reference category</li>
  <ul>
    <li>And the <b>slopes</b> are the relative <b>differences in means</b></li>
  </ul>
</ul>
...
<ul>
  <li>We need to talk about <b>factor variables</b> (another class of R objects) that is made for variables whose values <b>indicate</b> different <b>groups</b></li>
  <ul>
...
Going back to our race variable, we want to **order it** in a way that makes sense using the `relevel()` function:
...
Another option would have been to modify our `Race` variable directly in the regression call:
```{r, eval = F}
lm(Earnings ~ relevel(as.factor(Race), ref = "White"), asec)
```
...
```{r}
wid <- rio::import(here::here("lecture4", "slides_elements", "data", "wid.csv"))
```
...
<ul>
  ...
  <ul>
    <li>It simply means <b>converting</b> the discrete variables into dummies such that <b>everything</b> is <b>numeric</b></li>
  </ul>
</ul>
...
<ul>
  <li>But why do we need to <b>omit</b> one <b>category?</b></li>
  <ul>
...
<ul>
  <li>The regression does not fit well on the <b>y dimension</b></li>
</ul>
...
<ul>
  <li><center>The estimated probability density function has a long right tail</b></center></li>
</ul>
</ul>

<p style="margin-bottom:1.5cm"></p>

<center>&#10140; The distribution is likely <b>log-normal</b>! </center>
]
...
<center><b>Better with a log-transformation!</b></center>
...
<center>&#10140; But the fit looks weird for low and high number of hours</center>
]
...
<ul>
  <li>This positive <b>relationship</b> could be <b>driven</b> by a third variable (<b>omitted variable bias!)</b></li>
  <ul>
...
kable(tibble(` ` = c("Intercept", "Slope"),
       `\\(\\text{Male} = 0\\)` = c("\\(\\alpha\\)", "\\(\\beta_1 + 2\\beta_2\\text{Hours}\\)"),
       `\\(\\text{Male} = 1\\)` = c("\\(\\alpha + \\beta_3\\)", "\\(\\beta_1 + 2\\beta_2\\text{Hours}\\)")), 
      caption = "", align = "lcc")
```
]
...
```{r, echo = F, fig.width = 5, fig.height = 4, out.width=  "55%", eval = showasecplot}
...
ggplot(asec %>% mutate(fit = (Hours * coefs[2, 1]) + (Hours^2 * coefs[3, 1]) + coefs[1, 1],
  ...
  geom_point(alpha = .1) +
  geom_line(aes(y = fit)) +
  scale_color_manual(values = c("Female" = "coral", "Male" = "#00BFC4"))
```
...
<center><i>Actually the baseline coefficient was not that inflated</i></center>
...
kable(tibble(` ` = c("Intercept", "Slope"),
       `\\(\\text{Male} = 0\\)` = c("\\(\\alpha\\)", "\\(\\beta_1 + 2\\beta_2\\text{Hours}\\)"),
       `\\(\\text{Male} = 1\\)` = c("\\(\\alpha + \\beta_3\\)", "\\(\\beta_1 + 2\\beta_2\\text{Hours} + \\beta_4\\)")), 
      caption = "", align = "lcc")
```
]
...
<center><i>&#10140; A 4% difference</i></center>
]
...
<center><i>&#10140; Which is highly significant</i></center>
]
...
<ul>
  <li>\(\hat{\beta_1}\) is significantly <b>different from 0</b>, but could we test whether it is <b>different from a specific value</b>?</li>
  <li> We want to use our sample estimates to conclude something about the population parameter ! </li>
...
```{r, echo = FALSE}
concepts <- tibble(
  Concept = c("Null hypothesis", "Alternative hypothesis", "Test statistic",
              "P-value", "Significance level"),
  Description = c(
    "The hypothesis to evaluate",
    "The statement if the value differs from the null hypothesis",
    "The tool (point estimate statistic formula) we use to decide if we reject or not the null hypothesis",
    "The prob. of obtaining a test statistic just as extreme or more extreme than the observed test statistic assuming the null is true. *How surprised am I of observing \\(t\\) assuming \\(H_0\\) holds?*",
    "A cutoff on the p-value: if the p-value does not fall below \\(\\alpha\\) we would \"fail to reject\" \\(H_0\\)"
  ),
  Example = c(
    "\\(H_0: \\beta_1 \\geq .12\\)",
    "\\(H_1: \\beta_1 < .12\\)",
    "\\(t = \\dfrac{\\hat{\\beta} - .12}{s.e.(\\hat{\\beta})}\\)",
    "",
    "\\(\\alpha = 0.05\\)"
  )
)

kable(concepts, escape = FALSE, align = "lll", caption = "") %>%
...
<ul>
  ...
  <ul>
    <li> We need the value of the <b>area outside the interval</b> \(\left[-t;t\right]\), because we are testing a two-sided hypothesis </li>
    <li> Our t-stat shows that the event is unlikely to happen if the null hypothesis is true, but we need to quantify how unlikely it is </li>
  </ul>
</ul>
...
<ul>
  <li> Compute the <b>p-value</b>: the probability of getting sample evidence as or more extreme than what we observed, under the null hypothesis</li>
...
<ul>
  <li>Since <b>&alpha;</b> is the maximum p-value at which we reject <b>\(H_0\)</b>, we are ensuring that there is at most a <b>100 &times; &alpha;%</b> chance of making a <b>Type I error</b> (rejecting the null when it is true): a <b>5% chance</b> of mistakenly deciding that \(\beta_1 \geq .12\) when \(\beta_1 < .12\)</li>
</ul>
...
<ul>
  <li> We can use the linearHypothesis() function from the `{car}` package to perform t-tests</li>
  <ul>
    <li>The first argument is the <b>model</b></li>
    <li>The second argument is the <b>hypothesis/es</b></li>
  </ul>
</ul>
 
```{r, echo = F}
car::linearHypothesis(lm(lEarnings ~ Hours + sqHours + Sex + Hours * Sex, asec), c("Hours = .12"))
```
...
<ul>
  <li> We can also use it to carry out <b>F tests</b> for joint significance</li>
</ul>

--

```{r, eval = F}
car::linearHypothesis(lm(lEarnings ~ Hours + sqHours + Sex + Hours * Sex, asec), 
...
<center><a href = "https://economics.mit.edu/files/7782"><img src = "slides_elements/figures/table.png" width = "550"/></a></center>
...
<ul>
  <li><b>stargazer</b>: classic and easy, but unmaintained since ~2015

  <li><b>texreg</b>: same idea as <b>stargazer</b>, actively maintained, supports more model types.</li>

  <li><b>huxtable</b>: <b>huxreg()</b> makes quick regression tables that render to LaTeX, HTML, or Word unchanged.</li>

  <li><b>modelsummary</b>: most flexible and actively maintained; one call outputs to LaTeX/HTML/Word with easy customization.</li>

  <li><b>kableExtra</b>: can be used on top of other packages for further customization and styling.</li>
...
<ul>
  <li>Let's start with the <b>huxreg()</b> function from <b>huxtable</b>:</li>
  <ul>
    <li></li>
    <li></li>
    <li></li>
...
<ul>
  ...
  <ul>
    <li>As many <b>models</b> as you want, named or not</li>
    <li>Which <b>uncertainty statistic</b> to display (std.error, p.value, conf.low, conf.high)</li>
    <li></li>
    <li></li>
    <li></li>
    <li></li>
...
It can also be useful to provide a <b>graphical representation</b> of the coefficients

<ul>
  <li>You can do it with <b>dplyr</b> and <b>ggplot</b> extracting the elements from the regression output</li>
  <li>But there exists a <b>shortcut</b>: the <b>plot_summs()</b> function from the <b>jtools</b> package
    <ul>
    <li></li>
    <li></li>
    <li></li>
...
.pull-right[
<center><img src = "slides_elements/figures/pdf_output.png" width = "600"/></center>
]
