...
<ul>
  ...
  <ul>
    <li>Take-up of social programs</li>
    <li>Optimal taxation</li>
    <li>Intergenerational mobility</li>
  </ul>
</ul>
...
<ul>
  <li>You can reach me at <a href="mailto:elisa.alonsoh@gmail.com">elisa.alonsoh@gmail.com</a> for any question or comment about the course</li>
</ul>
]
...
<ul>
  <li>Course originally designed by <a href="https://www.louissirugue.com/teaching/">Louis Sirugue</a> </li>
</ul>
...
<ul>
  ...
  <ol>
    ...
    <li>Econometrics and Tables in R</li>
  </ol>
</ul>
]
...
<ul style = "margin-left:1.5cm;list-style: none">
  ...
  <ul style = "list-style: none">
    ...
    <li>1.3. R projects</li>
    <li>1.4. Packages</li>
    <li>1.5. Import and eyeball data</li>
  </ul>
</ul>
...
<ul style = "margin-left:-1cm;list-style: none">
  <li><b>3. The dplyr package</b></li>
  <ul style = "list-style: none">
    <li>3.1. Basic functions</li>
    <li>3.2. group_by() and summarise()</li>
  </ul>
</ul>
...
<ul>
  ...
  <ul>
    <li>An R project is simply a <b>folder</b> tied to a <b>.Rproj</b> file</li>
    <li>Opening the .Rproj file <b>starts a fresh R session</b> with the <b>working directory set to that folder</b></li>
  </ul>
</ul>
...
<ul>
  <li>Why bother? This is <b>good practice</b> for anything that is not a one-off script</li>
  <ul>
    <li>Your <b>scripts, data, and outputs</b> stay together in one place</li>
    <li>Your code becomes <b>portable</b>: it will run the same way on a collaborator's computer, or on your computer in five years</li>
    <li>You never need to use <code>setwd()</code> with an absolute path again</li>
  </ul>
</ul>
...
<center><i>&#10140; Create a project (File > New Project...) now for this course, and put today's script and data inside it</i></center>
...
<ul>
  <li>Base R is the core of the R programming language, but packages extend its capabilities (and make your life easier!):</li>
  <ul>
...
<ul> 
  <li>The package is now <b>installed on your computer</b>: you won't have to do it again</li> 
  <ul> 
    <li>But it is <b>not loaded in R yet</b>: each time you start a <b>new R session</b>, you'll have to load the packages you need with <b>library()</b></li> 
  </ul> 
</ul>
...
<ul> 
  <li>You can also use a package's functions without fully loading the package by using the <b>double colon</b> operator (<code>package::function</code>)</li> 
</ul>
...
<ul>
  <li>You can <b>download</b> this dataset from Konosys or from the <a href = "https://louissirugue.github.io/intro_to_R/home.html">original course webpage</a></li>
  <ul>
...
.pull-left[
<ul>
  <li>There are many ways to <b>import</b> stuff in R, but I recommend the package <code>rio</code></li>
  <ul>
    <li>It takes the <b>file directory</b> as an <b>input</b></li>
    <li>And gives the <b>file content</b> as an <b>output</b></li>
  </ul>
</ul>
]
...
<p style = "margin-bottom:1cm;"></p>
  ...
  <ul>
    <li>The function <code>rio::import()</code> chooses the right reader <b>based on the file extension</b> (.csv, .xlsx, .dta, .sav, ...)</li>
    <li>For <b>.csv</b> files, it uses <b>data.table::fread()</b> under the hood, which is <b>much faster</b> than base R's reader</li>
  </ul>
...
<center><b><i>&#10140; This works, but an absolute path is a bad idea... there's a better way!</i></b></center>
...
<ul>
  <li>An <b>absolute path</b> like <code>"C:/User/Documents/class/ligue1.csv"</code> only works <b>on your computer</b></li>
  <ul>
    <li>It <b>breaks</b> as soon as you share your script, or move your files around</li>
    <li>Depending on your operating system, it can also break if you use backslashes (\) instead of forward slashes (/)</li>
  </ul>
</ul>
...
<ul>
  <li>Since we are working inside an <b>R project</b>, we can use <b>relative paths</b> instead</li>
  <ul>
    <li>They are defined <b>from the project's root folder</b>, not from your computer's root</li>
  </ul>
</ul>
...
<ul>
  <li>The <code>here</code> package makes this effortless with the <b>here()</b> function</li>
  <ul>
    <li>It automatically finds your <b>project's root</b>, wherever your R script is opened from</li>
    <li>You just describe the path to your file <b>from there</b></li>
  </ul>
</ul>
...
```{r load-fb, include = FALSE}
fb <- rio::import(here::here("lecture1", "slides_elements", "data", "ligue1.csv"))
```
...
<b>Seems like it worked, but always check your data!</b>
</center>
