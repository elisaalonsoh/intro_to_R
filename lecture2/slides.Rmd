...
<ul>
  <li>Axes can be modified with <b>scale functions</b>, whose name depend on:</li>
  <ul>
...
<ul>
  <li>In some cases you may want to <b>convey information</b> with other means than position on an axis</li>
  <ul>
...
ggplot(wid, aes(x = inc_head, y = top1, color = continent)) + 
  geom_point(alpha = .3) 
```
...
#### 1) Reproduce that graph with the `starbucks` dataset
...
<ul>
  ...
  <ul>
    <li>You can make a them tell basically everything</li>
  </ul>
</ul>
