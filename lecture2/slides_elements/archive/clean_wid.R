...
wid <- read.csv("data/WID_Data_04072022-093826.csv", sep = ";") %>%
  select(-Percentile) %>% 
  group_by(Country) %>% 
  ...
  mutate(Country = ifelse(Country == "Cote d�???TIvoire", "Ivory Coast", Country),
         Country = ifelse(Country == "Cabo Verde", "Cape Verde", Country),
         Country = ifelse(Country == "Viet Nam", "Vietnam", Country)) %>%
       left_join(read.csv("data/continents.csv") %>%
