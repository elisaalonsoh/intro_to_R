...
<ul style = "margin-left:1.5cm;list-style: none">
  <li><b>1. Univariate regressions</b></li>
  <ul style = "list-style: none">
    <li>1.1. Continuous dependent variable</li>
    <li>1.2. Discrete dependent variable</li>
  </ul>
</ul>
...
<ul style = "margin-left:1.5cm;list-style: none">
  <li><b>2. Multivariate regressions</b></li>
  <ul style = "list-style: none">
    <li>2.1. Categorical dependent variable</li>
    <li>2.2. Interactions</li>
  </ul>
</ul>
...
<ul style = "margin-left:1.5cm;list-style: none"><li><b>5. Wrap up!</b></li></ul>
...
#### 1.1. Continuous dependent variable
...
#### 1.1. Discrete dependent variable
...
#### 2.1 Categorical dependent variable
...
petal_lm <- lm(`Log Earnings` ~ White + Other, asec)
graph_reso <- 0.05

#Setup Axis
axis_x <- seq(min(asec$White), max(asec$White), by = graph_reso)
...
plot_ly(asec, x = ~White, y = ~Other, z = ~`Log Earnings`) %>% 
...
.pull-left[
<p style = "margin-bottom:1cm"></p>
```{r, echo = F, fig.width = 8, fig.height = 5.5, out.width=  "100%"}
...
petal_lm <- lm(lEarnings ~ poly(Hours, 2, raw=TRUE) + Male, asec)
graph_reso <- 0.05

#Setup Axis
axis_x <- seq(min(asec$Hours), max(asec$Hours), by = graph_reso)
axis_y <- seq(min(asec$Male), max(asec$Male), by = graph_reso)
...
plot_ly(asec, x = ~Hours, y = ~Male, z = ~lEarnings) %>% 
...
<center>Actually the basline coefficient was not that much inflated</center>
...
<!-- -->
```{r, echo = F, fig.width = 7, fig.height = 1.5, out.width= '100%', fig.align="left", eval = showplotly}
petal_lm <- lm(lEarnings ~ poly(Hours, 2, raw=TRUE) + Male + Male * Hours, asec)
graph_reso <- 0.05
 ...
 * An alternative way to car linear hypothesis
 
```{r}
library(aod)

lm_results <- lm(lEarnings ~ Hours + sqHours + Sex + Hours * Sex, asec)

wald.test(vcov(lm_results), coef(lm_results), 2, H0 = .12, df = nrow(asec) - nrow(results)) %>% 
  print(digits = 7)
```
